...
import { PanGestureHandler, State, PanGestureHandlerEventPayload } from 'react-native-gesture-handler';
...
// Тип для события жеста с правильной типизацией nativeEvent
...
export default function HybridParticlesBackground({ 
  ...
  // Обработка касаний - исправленная версия с правильной типизацией
  const onGestureEvent = useCallback((event: any) => {
    if (!interactionEnabled) return;
    
    const nativeEvent = event.nativeEvent as PanGestureHandlerEventPayload & {
      x: number;
      y: number;
    };
