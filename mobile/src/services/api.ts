...
// Автоматически подставляет правильный URL для разработки/продакшена
...
// ОПРЕДЕЛЕНИЕ API URL
// ═══════════════════════════════════════════════════════
// В Expo Go (разработка): подключаемся к серверу на Linode
// В standalone APK (продакшен): тоже к серверу на Linode
// Всё централизовано на сервере!
// ═══════════════════════════════════════════════════════
...
const getApiUrl = () => {
  ...
  if (isStandalone) {
    // APK (скомпилированное приложение)
    return 'http://172.238.210.190:3001/api';
  } else {
    // Expo Go (разработка)
    return 'http://172.238.210.190:3001/api';
  }
